...
LargeImageButton::LargeImageButton(int id, const std::string& text) :
    ImageButton(id, text),
    field_74(1.0f)
{
    setupDefault();
...
LargeImageButton::LargeImageButton(int id, const std::string& text, const ImageDef& def) :
    ImageButton(id, text, def),
    field_74(1.0f)
{
    setupDefault();
}
...
void LargeImageButton::render(Minecraft* minecraft, int mouseX, int mouseY)
{
    if (!m_bVisible)
        return;
    ...
    if (texId >= 0)
    {
        ...
        if (tex)
        {
            ...
            if (hovered)
                field_74 = (field_74 - 0.025f < 0.9f) ? 0.9f : (field_74 - 0.025f);
            else
                field_74 = (field_74 + 0.025f > 1.0f) ? 1.0f : (field_74 + 0.025f);

            halfW = int(float(halfW) * field_74);
            halfH = int(float(halfH) * field_74);
